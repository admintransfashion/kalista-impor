...
/**
 * community/tfi/allotopup/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header allotopup (trn_allotopup)
...
$API = new class extends allotopupBase {
	...
	public function execute($options) {
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.allotopup_id LIKE CONCAT('%', :search, '%') OR A.allotopup_ref LIKE CONCAT('%', :search, '%') "
			];
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('allotopup_genby', 'allotopup_genby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
