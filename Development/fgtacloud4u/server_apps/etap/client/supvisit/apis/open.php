	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"supvisit_id" => " supvisit_id = :supvisit_id "
				]
			);

			$sql = \FGTA4\utils\SqlUtility::Select('trn_supvisit', [
				'supvisit_id', 'supvisit_descr', 'supvisit_datestart', 'supvisit_dateend', 'supvisit_iscommit', 'land_id', '_createby', '_createdate', '_modifyby', '_modifydate' 
			], $where->sql);
			...
			$result->record = array_merge($record, [
				'supvisit_datestart' => date("d/m/Y", strtotime($record['supvisit_datestart'])),
				'supvisit_dateend' => date("d/m/Y", strtotime($record['supvisit_dateend'])),
